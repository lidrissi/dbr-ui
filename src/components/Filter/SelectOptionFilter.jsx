...
const SelectOptionFilter = forwardRef((props, ref) => {
    ...
    subscription = notificationService
      ...
      .subscribe((widgetNotification) => {
        ...
        if (tag?.configuration?.dataSource?.query?.urlParams?.length > 0) {
          ...
          const oldValues = urlParams.map(
            (key) => notification.appParamsValue[key] || ""
          );
          const newValues = urlParams.map(
            (key) => widgetNotification.appParamsValue[key] || ""
          );
        ...
        notification = {
          ...widgetNotification,
          appParamsValue: { ...widgetNotification.appParamsValue },
        };
      });
    return () => {
  ...
  const changeCurrentSelection = (appParamsValue) => {
    if (
      tag.configuration.appParam?.length == 0 ||
      Object.keys(appParamsValue)?.length == 0
    ) {
      return;
    }
    Object.keys(appParamsValue).forEach((key) => {
      const tagAppParam = tag.configuration.appParam.find(
        ({ value }) => key == value
      );
      if (!tagAppParam) {
        return;
      }
      const selectedOption = tag.configuration.options.find(
        (option) => option[tagAppParam.label] == appParamsValue[key]
      );
      if (selectedOption) {
        setSelectedOption(
          selectedOption[tag.configuration.serviceParam] ||
      ...
      <DropdownMenu>
        {loading === false ? (
          options && options.length > 0 ? (
            <>
              <DropdownItem onClick={props.onReset}>ALL</DropdownItem>
              ...
              {options.map((option, index) => {
                return (
                  ...
                  >
                    {option[tag.configuration.serviceParam] ||
