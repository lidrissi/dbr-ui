...
const Filters = memo(({ filters, widgets }) => {
  ...
  const sendNotification = (notification) => {
    ...
    const notif = {
      ...notification,
      appParamsValue: { ...notification.appParamsValue },
    };
  ...
  useEffect(() => {
    ...
    let widgetInputParams = Object.keys(widgets)
      .map((key) => widgets[key].widget?.datasource)
    ...
    const filtersAppParams = filters.reduce(
      (acc, { configuration: { appParam } }) => {
        acc = [
          ...acc,
          ...(Array.isArray(appParam)
            ? appParam.map(({ value }) => value)
            : [appParam.value]),
        ];
        return acc;
      },
          ...
          tags.map((tag, index) => (
            <div className="filter-item" key={index}>
              <div className="filter-item__title d-flex justify-content-between">
                <span>{tag.name || tag.configuration.appParam?.label}</span>
                {tag.type == "inputText" ? (
                  <Button close onClick={() => removeFilterTag(index, tag)} />
                ) : null}
              </div>
              <span className="filter-item__value">
                {tag.type === "datePickerAvance" ? (
                  <DatePicker onSendNotification={sendNotification} tag={tag} />
                ) : tag.type === "inputText" ? (
                  <span
                    onClick={() =>
                      dispatchFilterNotification(
                        tag.configuration.inputText,
                        tag.configuration.appParam?.value
                      )
                    }
                  >
                    <InputText tag={tag} />
                  </span>
                ) : tag.type === "listeDeroulanteJson" ||
                  tag.type === "listeDeroulanteDataSource" ? (
                  <SelectOptionFilter
