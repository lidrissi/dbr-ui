...
function DatePicker(props) {
  ...
  useEffect(() => {
    let startDate =
      tag.configuration.startDateRange !== null
        ? new Date(tag.configuration.startDateRange)
        : dayjs().startOf("month").toDate();
    let endDate =
      tag.configuration.endDateRange !== null
        ? new Date(tag.configuration.endDateRange)
  ...
  const handleDateNotification = () => {
    ...
    if (
      appParam?.length == 2 &&
      mapParams.hasOwnProperty(appParam[0]?.value) &&
      mapParams.hasOwnProperty(appParam[1]?.value)
    ) {
      const startDate = Date.parse(
        mapParams[appParam[0].value].split("-").reverse().join("/")
      );
      const endDate = Date.parse(
        mapParams[appParam[1].value].split("-").reverse().join("/")
      );
  ...
  const dispatchDateFilterNotification = (
    notification,
    startDate,
    endDate,
    filter
    ...
    notification.appParamsValue[
      filter?.configuration?.appParam &&
        filter.configuration.appParam[0] &&
        filter.configuration.appParam[0].value
    ] = dayjs(startDate).format("DD-MM-YYYY");
    notification.appParamsValue[
      filter?.configuration?.appParam &&
        filter.configuration.appParam[1] &&
        filter.configuration.appParam[1].value
