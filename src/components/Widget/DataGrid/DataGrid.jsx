...
const DataGrid = memo(({ widget, onReceiveNotification }) => {
  ...
  const columns = (widget.mappingResponseParams || []).map(
    ({ responseParam, widgetParam }, index) => ({
      colIndex: index,
      name: responseParam,
      appParam: widget.mappingParams.find(
        (param) => widgetParam == param.widgetParam
      )?.appParam,
    })
  );
  ...
  const handleNotification = (notification) => {
    ...
    const commonAppParams = widget.mappingParams.filter(
      ({ appParam }) =>
        Object.keys(notification.appParamsValue).indexOf(appParam) > -1
    );
    let rowIndex = -1;
    if (commonAppParams?.length > 0) {
      rowIndex = widget.data.findIndex((rowData) =>
        commonAppParams.every(({ appParam, responseParam }) => {
          return (
            notification.appParamsValue[appParam] == rowData[responseParam]
          );
        })
      );
    }
  ...
  const components = {
    ExpandButton: function (props) {
      if (props.dataIndex !== expandableRowIndex || !urlParams)
