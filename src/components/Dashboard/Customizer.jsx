...
class Customizer extends React.PureComponent {
  ...
  setAppParams = (filters = []) => {
    (filters || []).forEach((filter) => {
      if (filter.type == "datePickerAvance" && filter.configuration.appParam) {
        ...
        mapParams[params[0].value] = formatDate(
          filter.configuration.startDateRange
        );
        mapParams[params[1].value] = formatDate(
          filter.configuration.endDateRange
        );
      } else if (
        filter.type == "inputText" &&
        filter.configuration?.appParam?.value
      ) {
        mapParams[filter.configuration.appParam.value] =
          filter.configuration.inputText;
      }
    });
  };
  ...
  componentDidUpdate(prevProps) {
    if (
      this.props.widgets != prevProps.widgets ||
  ...
  am4themes_dashboard = (target) => {
    ...
    if (
      target instanceof am4core.ColorSet &&
      appearance?.seriesColors?.length > 0
    ) {
      target.list = appearance.seriesColors.map((serieColor) =>
        am4core.color(serieColor)
      );
    }
    if (target instanceof am4core.InterfaceColorSet) {
      if (appearance?.widgetBackground) {
        ...
        target.setFor(
          "background",
          am4core.color(appearance?.widgetBackground)
        );
      ...
      <div key={key}>
        <div style={height}>
          <WidgetWrapper
            {...widgets[key]}
            showTitle={
              ["Image", "Text"].indexOf(widgets[key].widget?.stWidget?.type) ==
              -1
            }
          />
        </div>
    ...
    return (
      <DbrProvider>
        <Suspense fallback={<div />}>
          <Filters widgets={this.props.widgets} filters={this.props.filters} />
        </Suspense>
